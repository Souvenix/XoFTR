...
class FineProcessWithLine(nn.Module):
    def __init__(self, config):
        ...
        # 线特征处理模块
        self.line_feature_processor = nn.Sequential(
            nn.Conv2d(1, block_dims[0], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(block_dims[0]),
            nn.ReLU(inplace=True)
        )
        
        # 线特征融合模块
        self.line_feature_fusion = nn.Conv2d(block_dims[0] * 2, block_dims[0], kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, feat_f0, feat_f1, feat_m0, feat_m1, feat_c0, feat_c1, feat_c0_pre, feat_c1_pre, data):
        ...
        # 处理线特征
        if 'edge0' in data and 'edge1' in data:
            edge0 = data['edge0']
            edge1 = data['edge1']
            
            # 调整线特征分辨率以匹配细粒度特征
            edge0 = F.interpolate(edge0, size=feat_f0.shape[2:], mode='bilinear', align_corners=False)
            edge1 = F.interpolate(edge1, size=feat_f1.shape[2:], mode='bilinear', align_corners=False)
            
            # 提取线特征表示
            line_feats0 = self.line_feature_processor(edge0)
            line_feats1 = self.line_feature_processor(edge1)
            
            # 融合细粒度特征和线特征
            feat_f0 = self.line_feature_fusion(torch.cat([feat_f0, line_feats0], dim=1))
            feat_f1 = self.line_feature_fusion(torch.cat([feat_f1, line_feats1], dim=1))
            
            # 重新计算展开的特征，因为我们修改了原始的feat_f0和feat_f1
            stride_f = data['hw0_f'][0] // data['hw0_c'][0]
            feat_f0_unfold = F.unfold(feat_f0, kernel_size=(self.W_f, self.W_f), stride=stride_f, padding=self.W_f//2)
            feat_f0_unfold = rearrange(feat_f0_unfold, 'n (c ww) l -> n l ww c', ww=self.W_f**2)
            feat_f1_unfold = F.unfold(feat_f1, kernel_size=(self.W_f, self.W_f), stride=stride_f, padding=self.W_f//2)
            feat_f1_unfold = rearrange(feat_f1_unfold, 'n (c ww) l -> n l ww c', ww=self.W_f**2)
